...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.ShapeVar.set(0)
        Radiobutton(self.EntryFrame, text='圆', variable=self.ShapeVar, value = 0, command=self.DoIt,indicatoron=0,width=5,).grid(row=self.rownumber+1, column=0, sticky = W)
        Radiobutton(self.EntryFrame, text='矩形',   variable=self.ShapeVar, value = 2, command=self.DoIt,indicatoron=0,width=5,).grid(row=self.rownumber+2, column=0,sticky=W)

        self.stPS = Label(self.EntryFrame, text='Path',width=7)
        self.stPS.grid(row=self.rownumber, column=0,sticky=E)
        self.PathVar = IntVar()
        self.PathVar.set(0)
        Radiobutton(self.EntryFrame, text='并行', variable=self.PathVar, value = 0, command=self.DoIt ,indicatoron=0,width=7,).grid(row=self.rownumber+1, column=0, sticky = E)
        Radiobutton(self.EntryFrame, text='螺旋', variable=self.PathVar, value = 1, command=self.DoIt ,indicatoron=0,width=7,).grid(row=self.rownumber+2, column=0, sticky = E)

        self.stMove = Label(self.EntryFrame, text='补偿',width=7)
        self.stMove.grid(row=self.rownumber, column=1,sticky=W,padx=10)
        self.MovmentVar = IntVar()
        self.MovmentVar.set(0)
        Radiobutton(self.EntryFrame, text='Zero', variable=self.MovmentVar, value = 0, command=self.DoIt ,indicatoron=0,width=10,).grid(row=self.rownumber+1, column=1, sticky = W,padx=10)
        Radiobutton(self.EntryFrame, text='G41/G42', variable=self.MovmentVar, value = 1, command=self.DoIt ,indicatoron=0,width=10,).grid(row=self.rownumber+2, column=1, sticky = W,padx=10)

        self.stDir = Label(self.EntryFrame, text='方向',width=7)
        self.stDir.grid(row=self.rownumber, column=1,sticky=N)
        self.DirVar = StringVar()
        self.DirVar.set('CCW')
        Radiobutton(self.EntryFrame, text='逆时针', variable=self.DirVar, value = 'CCW', command=self.DoIt ,indicatoron=0,width=7,).grid(row=self.rownumber+1, column=1, sticky = N)
        Radiobutton(self.EntryFrame, text='顺时针', variable=self.DirVar, value = 'CW', command=self.DoIt ,indicatoron=0,width=7,).grid(row=self.rownumber+2, column=1, sticky = N)
              
        self.st13 = Label(self.EntryFrame, text='单位',width=5)
        self.st13.grid(row=self.rownumber, column=1,sticky=E)
        self.UnitVar = IntVar()
        self.UnitVar.set(1)
        Radiobutton(self.EntryFrame, text='英寸', variable=self.UnitVar, value = 0, command=self.Change_Units ,indicatoron=0,width=5,).grid(row=self.rownumber+1, column=1,sticky=E)
        Radiobutton(self.EntryFrame, text='毫米', variable=self.UnitVar, value = 1, command=self.Change_Units ,indicatoron=0,width=5,).grid(row=self.rownumber+2, column=1,sticky=E)
        ...
        self.PreambleVar.set('G17 G21 G90 M3 S600')
        ...
        #Tool
        self.rownumber += 1
        self.st10 = Label(self.EntryFrame, text='刀具直径')
        ...
        self.rownumber += 1
        self.st10a = Label(self.EntryFrame, text='刀具号(T)')
    ...
    def Change_Units(self):
        if (self.UnitVar.get() == 0) :
            #Inch 
            self.PreambleVar.set('G17 G20 G90 M3 S600 ')
        else :
            #MM
            self.PreambleVar.set('G17 G21 G90 M3 S600 ')
